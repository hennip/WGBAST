...
for(loop in 1:10){
  ...
  for(y in (yBreak+1):(nYears)){ 
    for(s in 1:length(stock_indices)){ 
      ...
      surv_migr[1,y,stock_indices[s],]<-1
      for(a in 2:6){  
        p.ladder[a,y,stock_indices[s],]<- apply(p.ladder[a,(yBreak-2):yBreak,stock_indices[s],],2,mean)
        surv_migr[a,y,stock_indices[s],]<- apply(surv_migr[a,(yBreak-2):yBreak,stock_indices[s],],2,mean)
      }
    } 
  }
  ...
  for(y in 1:(yBreak+3)){  #yBreak+3 =28+3=31 =2022
    for(s in 1:length(stock_indices)){   
      ...
      x<-paste0("SmoltW[",y+years[1]-1987,",",s,"]")   #years[1]=1992  so this is y+5
  ...
  for(i in 1:yBreak){
    for(a in 1:2){ # grilse/MSW
      #i<-1;a<-1
      qdw<-paste0("qdW[",(i+4-(a-1)),",",(a+1),"]")
      qdr<-paste0("qdR[",(i+4-(a-1)),",",(a+1),"]")   
      qlw<-paste0("qlW[",(i+4-(a-1)),",",(a+1),"]")
      qlr<-paste0("qlR[",(i+4-(a-1)),",",(a+1),"]")        
    ...
    htw<-paste0("HtW[",i+4,",2]")
    htr<-paste0("HtR[",i+4,",2]")
  ...
  for(y in 1:yBreak){    
    for(a in 1:6){
      ...
      # this seems like it's missing a fix from smolt cohort to calendar year!?!
      x<-paste0("HrW[",(y+5),",",a,"]")
      xR<-paste0("HrR[",(y+5),",",a,"]")
      for(r in 1:Nstocks){
  ...
  for(y in 1:yBreak){
    ...
    for(r in 1:Nstocks){
      WOLL_HRtmp[2,y,r,1,]<-   1-exp(-qlW[1,y,]* EffortAssesUnit[,y,1,"OLL",])
      WOLL_HRtmp[3:6,y,r,1,]<- 1-exp(-qlW[2,y,]* EffortAssesUnit[,y,1,"OLL",])
      WODN_HRtmp[2,y,r,1,]<-   1-exp(-qdW[1,y,]* EffortAssesUnit[,y,1,"ODN",])
      WODN_HRtmp[3:6,y,r,1,]<- 1-exp(-qdW[2,y,]* EffortAssesUnit[,y,1,"ODN",])     
      WTR_HRtmp[2:6,y,r,1,]<-HtW[y,]   
    }
    for(u in 1:4){
      ROLL_HRtmp[2,y,u,1,]<-   1-exp(-qlR[1,y,]* EffortAssesUnit[,y,1,"OLL",])
      ROLL_HRtmp[3:6,y,u,1,]<- 1-exp(-qlR[2,y,]* EffortAssesUnit[,y,1,"OLL",])    
      RODN_HRtmp[2,y,u,1,]<-   1-exp(-qdR[1,y,]* EffortAssesUnit[,y,1,"ODN",])
      RODN_HRtmp[3:6,y,u,1,]<- 1-exp(-qdR[2,y,]* EffortAssesUnit[,y,1,"ODN",])
      RTR_HRtmp[2:6,y,u,1,]<-HtR[y,]   
    }
    ...
    for(u in 1:4){
      ...
      RCDN_HRtmp[3:6,y,u,2,]<-1-exp(-qdR[2,y,]* EffortAssesUnit[,y,1,"CDN",])
    ...
    for(r in 1:Nstocks){
      ...
      WCDN_HRtmp[3:6,y,r,2,]<- 1-exp(-qdW[2,y,]*EffortAssesUnit[,y,AU[r],"CDN",])
    ...
    WsalmStock[,y,,1,]<-WsalmStock[,y,,1,]*exp(-(WsalmNatMort[,y,,1,]/12))-WTR_Ctmp[,y,,1,] 
