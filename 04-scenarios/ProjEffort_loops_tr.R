...
# ~*~*~*~*~*~*~*~*~~*~*~*~*~*~*~*~*~~*~*~*~*~*~*~*~*~~*~*~*~*~*~*~*~*~~*~*~*~*
  ...
  # for(y in (yBreak+1):(yBreak+3)){
    ...
    WTR_HR[,y,,]<-WTR_HR[,yBreak,,]*CoefTrollingF
    RTR_HR[,y,,]<-RTR_HR[,yBreak,,]*CoefTrollingF
    ...
    WOLL_HR[2,y,,]<- 1-exp(-ql_W[1,y,]* EffortAU[y,1,"OLL",])           #grilse
    WOLL_HR[3:6,y,,]<- 1-exp(-ql_W[2,y,]* EffortAU[y,1,"OLL",])        #2 msw
  ...
  #  print(MatW_3[,,1,1])
    ...
    for(a in 1:1){
      for(r in 1:Nstocks){
        ...
        ImmW_3[a,y,r,]<-ImmW_2[a,y,r,]*exp(-(MW[a,y,r,1,]*(1/12)))-WTR_C[a,y,r,]
      }                  
      for(u in 1:4){
    ...
    for(a in 2:6){
      ...
      ImmW_3[a,y,,]<-ImmW_2[a,y,,]*exp(-(MW[a,y,,1,]*(1/12)))-WTR_C[a,y,,]
  ...
  for(i in 1:nsim){
    for(y in 1:Nyears){
      ...
      if(y>1){
        ...
        CalC_tot[y,i]<-C_OLL[y-1,i]+C_CTN[y,i]+C_TR[y,i]
      }
    }
  }
