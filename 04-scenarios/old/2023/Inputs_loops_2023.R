...
#  loop<-1
  ...
  # under these names 
  BH_dataFile<-
      paste0(PathOut_Scen, "ScenHist_JAGSmodel", Model,"_",loop,".RData") # name to separate historical part from future projections?
  ...
  #  for(y in 1:(yBreak)){ # For 2020 assessment only!!! 
      #y<-yBreak-4
      x<-paste0(PathData_Scen,"M74/M74[" ,y+years[1]-1987,"].txt")
    i<-read.table(x)
    if(loop<11){
