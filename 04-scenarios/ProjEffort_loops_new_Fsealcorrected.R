...
# loop<-1    
    ...
    ql_W[,sims[1]:sims[2]]<-qlW[,]
    ql_R[,sims[1]:sims[2]]<-qlR[,]
...
# ~*~*~*~*~*~*~*~*~~*~*~*~*~*~*~*~*~~*~*~*~*~*~*~*~*~~*~*~*~*~*~*~*~*~~*~*~*~*
  ...
  # for(y in (yBreak+1):(yBreak+3)){
    ...
    WRF_HR[,y,,]<- WRF_HR[,yBreak,,]*CoefRiverF          #wild  river fishing
    RRF_HR[,y,,]<- RRF_HR[,yBreak,,]*CoefRiverF          #reared
    ...
    WOLL_HR[2,y,,]<- 1-exp(-ql_W[1,]* EffortAU[y,1,"OLL",])           #grilse
    WOLL_HR[3:6,y,,]<- 1-exp(-ql_W[2,]* EffortAU[y,1,"OLL",])        #2 msw
    ROLL_HR[2,y,,]<- 1-exp(-ql_R[1,]* EffortAU[y,1,"OLL",])
    ROLL_HR[3:6,y,,]<- 1-exp(-ql_R[2,]* EffortAU[y,1,"OLL",])
    ...
    for(u in 1:4){
      ...
      RCTN_HR[3:6,y,u,]<- 1-exp(-qctn_R[2,u,]* EffortAU[y,u,"CTN",])
    }
    
    for(r in 1:Nstocks){
      WCTN_HR[2,y,r,]<- 1-exp(-qctn_W[1,AU[r],]* EffortAU[y,AU[r],"CTN",])
      WCTN_HR[3:6,y,r,]<- 1-exp(-qctn_W[2,AU[r],]* EffortAU[y,AU[r],"CTN",])
    }
    ...
    for(a in 1:6){
      ...
      for(u in 1:4){
        ...
        MatR_2[a,y,u,]<-(MatR_1[a,y,u,]*exp(-(MR[a,y,u,2,]*F_seal[y,a,u]*(1/12)))*
          exp(-(MR[a,y,u,2,]*(1/12)))-RCTN_C[a,y,u,])*exp(-(MW[a,y,r,2,]*F_seal[y,a,AU[r]]*(1/12)))
     # Second month of MW with F_seal moved here from August, corresponds now to Inputs and JAGS model
      }  
    }
 ...
 #   print(MatW_2[,,1,1])
    ...
    # (MatW_3 below same as NspW in JAGS model) 
  ...
  #  print(MatW_3[,,1,1])
    ...
    if(y<(yBreak+NumFutYears)) {
      May1stW[2:6,y+1,,]<-ImmW_2[1:5,y,,]*exp(-(MW[1:5,y,,1,]*(3/12)))
      May1stR[2:6,y+1,,]<-ImmR_2[1:5,y,,]*exp(-(MR[1:5,y,,1,]*(3/12)))
    }
    # 3/12: Feb-April
    ...
    if(y<(yBreak+NumFutYears)){
      for(s in 1:nsim){
        for(r in 1:Nstocks){   #STOCK
          ...
          error<-exp(rnorm(1, sd = sqrt(1/tauBH[s])))        #mean is 0
          
          #In unit 4 the smolts recruit a year earlier
          if(r>13 & r<16){
            May1stW[1,(y+1),r,s]<-as.numeric(E*error/(BHalpha[s,r]+BHbeta[s,r]*E))
          }else{ #stocks other than 14 & 15
            
            if(y<(yBreak+NumFutYears-1)){          # for(y in (yBreak+1):(yBreak+NumFutYears))
              May1stW[1,(y+2),r,s]<-as.numeric(E*error/(BHalpha[s,r]+BHbeta[s,r]*E))
            }
          }
        }
      }
  ...
  for(a in 1:2){
    ...
    OffsW_HR[a,,]<- 1-exp(-(-log(1-WODN_HR[a+1,,1,])-log(1-WOLL_HR[a+1,,1,])))
    OffsR_HR[a,,]<- 1-exp(-(-log(1-RODN_HR[a+1,,1,])-log(1-ROLL_HR[a+1,,1,])))
  ...
  for(s in 1:nsim){
    for(y in 1:Nyears){
        ...
        for(r in 1:Nstocks){
          ...
          PSW[r,y,i]<-May1stW[1,y,r,s]*exp(-(MW[1,y,1,1,s])) # post-smolts
        ...
        Prop1SWsp[y,i]<-sum(MatW_3[2,y,,s])/sum(MatW_3[,y,,s])
        Prop2SWsp[y,i]<-sum(MatW_3[3,y,,s])/sum(MatW_3[,y,,s])
        Prop3SWsp[y,i]<-sum(MatW_3[4,y,,s])/sum(MatW_3[,y,,s])
        Prop4SWsp[y,i]<-sum(MatW_3[5,y,,s])/sum(MatW_3[,y,,s])
