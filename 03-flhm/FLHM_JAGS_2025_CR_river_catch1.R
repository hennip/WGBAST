...
for (i in 1:(m+proj_years)){             
    ...
    # Coastal fish in the beginning of July in year 1      
    NcRsp[i,1,s] <- NrRsp[i,1,s]*(1-HrW[i,1,s])*exp(-MpsR[i]*sealMort[i,1,1]/Tstep)*kE[i,1,5]
    ...
    for (j in 2:6){ 
        ...
        # Number of spawners in the river in the beginning of October    #Mature
        NspRsp[i,j,s] <- NladderRsp[i,j,s]*(1-HrW[i,j,s])* exp(-2*MR/Tstep)*surv_migr[(i+j-1),s]*kE[i,j,5]	
        }
    }
}
...
#Post-smolts
        ...
        TcW[i,1,s] <- TrW[i,1,s]*surv.rivAU[i,1,s]*kE[i,1,5]	#ATTN!! surv.riv[i,1,AU_stock[s]]?? or leave as is....see process errors
        TcR[i,1,s] <- TrR[i,1,s]*surv[i,1,5,2]*kE[i,1,5]	      
        TcRsp[i,1,s] <- TrRsp[i,1,s]*(1-HrWAU[i,1,s])*exp(-MpsR[i]*sealMort[i,1,1]/Tstep)*kE[i,1,5]	  
        ...
        TspW[i,j,s]<-TrW[i,j,s]*surv.rivAU[i,j,s]*kE[i,j,5]		   #!!ATTN what to do here
        TspR[i,j,s]  <- TrR[i,j,s]*surv[i,j,5,2]*kE[i,j,5]		
        TspRsp[i,j,s] <- TrRsp[i,j,s]*(1-HrWAU[i,j,s])* exp(-2*MR/Tstep) *kE[i,j,5]	
        }			
    }
}
...
# }   
    ...
    # River fishery
        ...
        cr_predW[i,j] <- (TrW[i,j,1]*HrWAU[i,j,1]+TrW[i,j,2]*HrWAU[i,j,2]+TrW[i,j,3]*HrWAU[i,j,3]+TrW[i,j,4]*HrWAU[i,j,4])* reportrW
        psi_rW[i,j]<-rrW[j]/(rrW[j]+cr_predW[i,j])
        cr_ObsW[i,j] ~ dnegbin(psi_rW[i,j], rrW[j])
         
        cr_predRsp[i,j] <-  (TrRsp[i,j,1]*HrWAU[i,j,1]+TrRsp[i,j,2]*HrWAU[i,j,2]+TrRsp[i,j,3]*HrWAU[i,j,3]+TrRsp[i,j,4]*HrWAU[i,j,4])* reportrW   
    ...
    # Coastal fishery
       ...
       for(s in 1:2){
          ncrW[i,j,s]<-HrW[i,j,s]*NladderW[i,j,s]+NladderRsp[i,j,s]*HrW[i,j,s]   
       }
       for(s in 3:stocks){
...
for(i in 1:(m+proj_years)){
    ...
    surv[i,1,5,1]<-1 #(1-HrW[i,1])*exp(-MpsW[i]*sealMort[i,1,1]/Tstep)    #survrW apply AU 1 seal M 
    surv[i,1,5,2]<-(1-HrR[i,1])*exp(-MpsR[i]*sealMort[i,1,1]/Tstep)    #survrR
   
   for(s in 1:stocks){
       surv.riv[i,1,s] <- (1-HrW[i,1,s])*exp(-MpsW[i]*sealMort[i,1,1]/Tstep) 
   }
       
    HrWAU[i,1,1] <-  mean(HrW[i,1,au1_stocks])
    HrWAU[i,1,2] <- mean(HrW[i,1,au2_stocks])
    HrWAU[i,1,3] <- mean(HrW[i,1,au3_stocks])
    HrWAU[i,1,4] <- mean(HrW[i,1,au4_stocks])
    
      for(s in 1:AUS){
           surv.rivAU[i,1,s] <- (1-HrWAU[i,1,s])*exp(-2*MW/Tstep)     #survrW
        }		                
    ...
    for(j in 2:6){
        ...
        for(s in 1:stocks){
           surv.riv[i,j,s] <- (1-HrW[i,j,s])*exp(-2*MW/Tstep)     #survrW
        }
         
    HrWAU[i,j,1] <- mean(HrW[i,j,au1_stocks])
    HrWAU[i,j,2] <- mean(HrW[i,j,au2_stocks])
    HrWAU[i,j,3] <- mean(HrW[i,j,au3_stocks])
    HrWAU[i,j,4] <- mean(HrW[i,j,au4_stocks])
    
      for(s in 1:AUS){
           surv.rivAU[i,j,s] <- (1-HrWAU[i,j,s])*exp(-2*MW/Tstep)     #survrW
        }
...
##########################################################################################
  for(s in 1:stocks){  #prior for harvest rate age 3/2SW in the river
  
  #HrW[1,1,s]<-HrW[3,1,s] 
  #HrW[2,1,s]<-HrW[3,1,s] 
  #for(i in 3:m){	
  #   HrW[i,1,s] <- HrW[i+1-3,3,s]*deltaHrW1 #1SW HR relative to 2SW HR same for all stocks for now
  #}
  
  HrW[1,2,s]<-HrW[2,2,s] 
  for(i in 2:m){	
     HrW[i,2,s] <- HrW[i+2-3,3,s]*deltaHrW #1SW HR relative to 2SW HR same for all stocks for now 
  }
  for(i in 1:m){	
     HrW[i,1,s] <- 1-exp(-(qrW[1]*Er[i,1]))  #change this??

     for (j in 4:6){ 
        HrW[i,j,s] <- HrW[i+j-3,3,s] #!!!! equal by year not by smolt cohort
     }
  }
  mu_HrW[s]~dnorm(-1.3862944 ,0.5932593) #check!!
  HrW_autoc[s]~dunif(0.1,0.99)
  THrW[s]<-1/((CV_HrW*mu_HrW[s])*(CV_HrW*mu_HrW[s]))
  
  for(i in 1:(m+3)){
      cov_HrW[i,i,s]<-1/THrW[s]
      mu_HrWt[i,s]<-mu_HrW[s]
      HrW[i,3,s]<-ilogit(logit_HrW[i,3,s])
   
  }
  logit_HrW[1:(m+3),3,s]~dmnorm(mu_HrWt[1:(m+3),s],tauHrW[1:(m+3),1:(m+3),s])
  tauHrW[1:(m+3),1:(m+3),s]<-inverse(cov_HrW[1:(m+3),1:(m+3),s])

for(i in 1:(m+2)){
   for(k in (i+1):(m+3)){
      cov_HrW[i,k,s]<-cov_HrW[i,i,s]*pow(HrW_autoc[s],abs(i-k))
      cov_HrW[k,i,s]<-cov_HrW[i,k,s]
   }
}
...
CV_HrW~dunif(0.01,1)             
logit_deltaHRW~dnorm(0.85,0.25)
deltaHrW<-ilogit(logit_deltaHRW)
