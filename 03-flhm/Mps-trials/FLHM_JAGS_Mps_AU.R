...
#Post-smolts
    ...
    NrW[i,1,s] <- SmoltW[i,s]*survMps[i,1,s]  #*kE[i,1,1]    #SmoltW beginning of May (surv[i,1,1,1] 1 month)

    # Coastal fish in the beginning of July in year 1      
    NcW[i,1,s] <- NrW[i,1,s]*survMps[i,5,s] #*kE[i,1,5]
    
   # Offshore salmon in the beginning of January in year 1 (calendar year 2)
    NdoW[i,1,s] <- NcW[i,1,s]*survMps[i,3,s] #*kEc[i,1] 
   	
    # Offshore salmon in the beginning of February in year 1  (calendar year 2)
    NlW[i,1,s] <- NdoW[i,1,s]*survMps[i,4,s] #*kE[i,1,4]
   																													
	# Offshore salmon in the beginning of April in year 1  (calendar year 2)
    NtW[i,1,s] <- NlW[i,1,s]*survMps[i,2,s] #*kE[i,1,2]		
...
MR ~dlnorm(-2.3, 4.3)T(0.025,0.35)
   ...
   for(st in 1:AUS){                  
        ...
        logit_survMps[1,st] ~ dnorm(0,0.1)T(-3,-0.4)  #max ~40%
    ...
    for(i in 1:AUS){
        ...
        R[2,2]     <- sqrt(1 - r12^2)   
        for(i in 2:AUS){
          R[i,1]   <- 0    #with > 2 AUS (plus lines below)
        }  
          for (i in 2:(AUS-1)) {         #NB AUS needs to be >2, otherwise just use lines above!!
            ...
            for(j in 1:i){
              scZ[i, j]  <- corZ[i, j] / sqrt(inprod(corZ[i, 1:i], corZ[i, 1:i]))
              R[j,(i+1)]   <- sqrt(corY[i]) * scZ[i,j]
            }
            for(j in (i+1):AUS){ 
              R[j,i] <- 0
            }
  ...
  for(i in 1:(m)){
    for(s in 1:stocks){
            ...
            survMps[i,5,s] <- (1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsW[i,AU[s]]*sealMort[i,1,1]/Tstep)  #was surv.riv
            survMps[i,6,s]<-(1-HtW[i,1]*(p.rel[(i+1-1)]*p.mort+(1-p.rel[(i+1-1)])))* exp(-MpsW[i,AU[s]]/Tstep) 
        }
    
    RMps[i]~dbeta(Ra,Rb) 
    ReffectMps[i] <- (RMps[i] * 1.5)+1         

    MpsR[i] <- MpsW[i,1] * ReffectMps[i]
    survMpsR[i]<-exp(-MpsR[i])             
    
    #surv[,1,,1] are now replaced in code by survMps
    ...
    for(s in 1:stocks){
       surv.riv[i,1,s] <- (1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsW[i,AU[s]]*sealMort[i,1,1]/Tstep) #not used anywhere
   }		                
...
for(i in 1:(m+proj_years)){
    for(j in 1:6){           
        ...
        zz[i,j,5]~ dgamma(2, 50)I( , maxvar)  #for survt
		# 6 survt
        for(ii in 6:6){
          ...
          minv[i,j,ii]<-min(vvE[i,j,ii,1],vvE[i,j,ii,2])      #compare W with R
          vE[i,j,ii]<-mon_E[j,ii]*zz[i,j,(ii-1)]/12
          maxError[i,j,ii] <- 1 + pow((3 * min(vE[i,j,ii], minv[i,j,ii])), 0.5)   #variance depending on time step
          minError[i,j,ii] <- 1 - pow((3 * min(vE[i,j,ii], minv[i,j,ii])), 0.5) 
...
for(j in 2:3){
mu_qdW[1,j]<-1 #not used just avoids error during initialization
  for(i in 1:(m+proj_years+3)){
