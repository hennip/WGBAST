...
MR ~dlnorm(-2.3, 4.3)T(0.025,0.35)
   ...
   for(st in 1:stocks){                  
    ...
    for(i in 1:stocks){
        ...
        R[2,2]     <- sqrt(1 - r12^2)   
        for(i in 2:stocks){
          R[i,1]   <- 0    #with > 2 stocks (plus lines below)
        }  
  ...
  for(i in 1:(m)){
    for(s in 1:stocks){
            ...
            survMps[i,5,s] <- (1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsW[i,s]*sealMort[i,1,1]/Tstep)  #was surv.riv
    ...
    for(s in 1:stocks){
       surv.riv[i,1,s] <- (1-HrW[i,1]*rivHR[(i+1-1),s])*exp(-MpsW[i,s]*sealMort[i,1,1]/Tstep) #not used anywhere
   }		                
