...
#    mu_SR[s]<- -0.5/tau_SR[s]
  ...
  for (s in 1:stocks){
    ...
    for (i in 1:(m)){              
      ...
      #NcW[i,1,s] <- NrW[i,1,s]*surv[i,1,5,1]*kE[i,1,5]
      ...
      for (j in 2:6){ 
        ...
        #NspW[i,j,s]<-NladderW[i,j,s]*surv[i,j,5,1]*surv_migr[(i+j-1),s]*kE[i,j,5]
  ...
  for(s in 1:AUS){              #s index denotes AU here 
    for (i in 1:m){ 
      ...
      TcW[i,1,s] <- TrW[i,1,s]*surv[i,1,5,1]*kE[i,1,5]	#ATTN!! surv.riv[i,1,AU_stock[s]]?? or leave as is....see process errors
      ...
      for (j in 2:6){ 
        ...
        TspW[i,j,s]<-TrW[i,j,s]*surv[i,j,5,1]*kE[i,j,5]		   #!!ATTN what to do here
  ...
  for (i in 1:m){                    ### Tagging data likelihoods ### 
    ...
    for(j in 1:5){     #check - seems ok because all immature fish
      ...
      psi_lR[i,j]<-min(rlR[j]/(rlR[j]+cl_predR[i,j]),0.99)
    ...
    for (j in 1:6){
      ...
      TdcWtot[i,j]<- sum(TdcW[i,j,1:AUS])    
      TdcRtot[i,j] <- sum(TdcR[i,j,1:AUS]) + sum(TdcRsp[i,j,1:AUS])	     
      
      cdc_predW[i,j] <- TdcWtot[i,j]*HdcW[i,j]*reportd*Tretain           
      psi_dcW[i,j]<-min(rdW[j]/(rdW[j]+cdc_predW[i,j]),0.99)
      #cdc_ObsW[i,j] ~ dnegbin(psi_dcW[i,j], rdW[j])	
      
      cdc_predR[i,j] <- TdcRtot[i,j]*HdcR[i,j]*reportd*Tretain     
      psi_dcR[i,j]<-min(rdR[j]/(rdR[j]+cdc_predR[i,j]),0.99)
      #cdc_ObsR[i,j] ~ dnegbin(psi_dcR[i,j], rdR[j])	 
      
      # River fishery
      
      TrWtot[i,j]<- sum(TrW[i,j,1:AUS])  # Sum of tagged salmon over k areas
      TrRsptot[i,j]<- sum(TrRsp[i,j,1:AUS])  # Sum of tagged salmon over k areas			
      TrRtot[i,j] <- sum(TrR[i,j,1:AUS]) # Sum of tagged salmon over k areas	
      
      cr_predW[i,j] <- TrWtot[i,j]* HrW[i,j]* reportrW    #ATTN     
      psi_rW[i,j]<-min(rrW[j]/(rrW[j]+cr_predW[i,j]),0.99)
      cr_ObsW[i,j] ~ dnegbin(psi_rW[i,j], rrW[j])
      
      cr_predRsp[i,j] <- TrRsptot[i,j]*HrW[i,j]* reportrW   #ATTN            # note uses reared surv but wild HR        
      psi_rRsp[i,j]<-min(rrRsp[j]/(rrRsp[j]+cr_predRsp[i,j]),0.99)
      cr_ObsRsp[i,j] ~ dnegbin(psi_rRsp[i,j], rrRsp[j])
      
      cr_predR[i,j] <- TrRtot[i,j]*HrR[i,j]*reportrR        
      psi_rR[i,j]<-min(rrR[j]/(rrR[j]+cr_predR[i,j]),0.99)
      ...
      psi_cW[i,j,1]<-min(rcW[j]/(rcW[j]+cc_predW[i,j,1]),0.99)
      ...
      for(au in 1:AUS){   
        ...
        psi_cR[i,j,au]<-min(rcR[j]/(rcR[j]+cc_predR[i,j,au]),0.99)
      ...
      #ncr[i,j]<- HrW[i,j]*NrW_all[i,j]+HrR[i,j]*NrR_all[i,j]
...
#  early_MpsW~T(dlnorm(0.23,19),0.5,5)
  ...
  #CV_MpsW ~T(dbeta(1,1),0.001,0.999)
  CV_MpsW ~dlnorm(-1.20,2)
  
  #smooth changes in Mps
  
  # LM[1:m]~dmnorm(muLogM[1:m],tauLM[1:m,1:m])
  # for(t in 1:m){
  #       muLogM[t]<-log(early_MpsW)-0.5/TLM
  #       MpsW[t]<-exp(LM[t])
  #       covM[t,t]<-1/TLM
  #    }
  ...
  # CVM~dunif(0.01,1)
  # TLM<-1/log(CVM*CVM+1)
  # M_autoc~dunif(0.3,0.99)
  
  # tauLM[1:m,1:m]<-inverse(covM[1:m,1:m])
  # for(t in 1:(m-1)){
  #    for(k in (t+1):m){
  #       covM[t,k]<-covM[t,t]*pow(M_autoc,abs(t-k))
  #       covM[k,t]<-covM[t,k]
  #    }
  # }
  ...
  for(i in 1:(m)){
    for(j in 1:6){           
      for(ii in 1:4){
        ...
        #zz[i,j,ii]~ T(dgamma(2, 50),0.001,maxvar)
        zz[i,j,ii]~ dunif(0.0001,maxvar)
      ...
      #5 survr  
     # zzr[i,j]~ T(dgamma(2, 50),0.001,maxvar)        
       zzr[i,j]~ dunif(0.0001,maxvar)   
      
      for(s in 1:stocks){  
        
       # kkrw[i,j,s] <- step(surv[i,j,5,1]*surv_migr[(i+j-1),s] - 0.5) # if surv >=0.5 then kk=1 else kk=0 
        #maxErw[i,j,s] <- ((1 / (surv[i,j,5,1]*surv_migr[(i+j-1),s])) * kkrw[i,j,s]) + (2 * (1 - kkrw[i,j,s]))  
      ...
      minvrW[i,j]<-min(vvErw[i,j,1:stocks])
      minvr[i,j]<-min(minvrW[i,j],vvErr[i,j])      #compare W with R
     ...
     # zzc[i,j]~ T(dgamma(2, 50),0.001,maxvar)         #survc
     # zzdc[i,j]~ T(dgamma(2, 50),0.001,maxvar)         #survdc
       zzc[i,j]~ dunif(0.0001,maxvar)         #survc
      zzdc[i,j]~ dunif(0.0001,maxvar)         #survdc
  ...
  phi_tr~dunif(0,0.99) #positive autocorrelation
  ...
  phi_ql~dunif(0,0.99) #positive autocorrelation
  ...
  phi_qd~dunif(0,0.99) #positive autocorrelation
  ...
  #priors for spawner counting
    ...
    logit_mu_spawn[s]~T(dnorm(mu_mu_sp[s],tau_mu_sp[s]),0.85,) #lower bound 0.70
    #logit_CV_spawn[s]~T(dnorm(mu_CV_sp[s],tau_CV_sp[s]),,-1.75)  #upper bound 0.15
    logit_CV_spawn[s]~T(dnorm(mu_CV_sp[s],tau_CV_sp[s]),,-2.44)  #upper bound 0.08
...
#   for(s in 3:3){
#  
#    logit_mu_spawn[s]~T(dnorm(mu_mu_sp[s],tau_mu_sp[s]),0,) #lower bound~0.50
#    logit_CV_spawn[s]~T(dnorm(mu_CV_sp[s],tau_CV_sp[s]),,-0.4)  #upper around 0.4
#    
#    mu_spawn[s]<-ilogit(logit_mu_spawn[s]) 
#    CV_spawn[s]<-ilogit(logit_CV_spawn[s]) 
#    
