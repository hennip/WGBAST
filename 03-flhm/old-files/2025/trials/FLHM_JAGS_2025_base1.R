# FLHM_JAGS_2025_base -model contains 
# - changes to Simo Didson count fitting (only MSW salmon, CoefDS without hiearachy)
# - Råne spawner count included in data (no changes to model structure)
# Also,
# M74 values for 2023 spawners/2024 hatched are recycled from year before in the absence of finnish hatching data

# truncation of Tornios p.detect by using normal distribution and modified expit
...
for(s in 1:stocks){
    ...
    #     p.detect[i,s]~dbeta(a_spawn[s],b_spawn[s])#T(0.5, )         #extra 5 years (m+5) not used here...
...
#   only for tornio to allow truncate
#   truncate done by switching beta -> normal
#   modified expit to limit between 0.5 - 1
#   had to add dnorm to remain as random variable (not deterministic)
#   very tight normal with sd of 0.000000001
#   by doing this p.detect SHOULD be exactly same as its mu
#   and mu is from modified expit between 0.5 - 1
#   has a really slight CHANGE to deviate from mu, SHOULD not be possible


for(i in 1:(m+proj_years+5)){
      #p.detect[i,1]~dbeta(a_spawn[1],b_spawn[1])#T(0.1, )         #extra 5 years (m+5) not used here...
      
      p.detect[i,1] ~dnorm( 0.5*(1/(1+exp(-p.detect_n[i,1])))+0.5, 0.000000001^-2)
      
      p.detect_n[i,1] ~ dnorm(logit(mu_spawn[1]), 
      abs(mu_spawn[1]*CV_spawn[1]+10^-8)^-2)
      
      p.ladder[i,1]~dbeta(alpha_ladder[i,1],beta_ladder[i,1])
      surv_migr[i,1]~dbeta(alpha_migr[i,1],beta_migr[i,1])   
  }
