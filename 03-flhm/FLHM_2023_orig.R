...
#Post-smolts
        ...
        TcW[i,1,s] <- TrW[i,1,s]*surv[i,1,5,1]*kE[i,1,5]	     
        ...
        NspR[i,j,s]  <- NrR[i,j,s]*surv[i,j,5,2]*kE[i,j,5]	
			  ...
			  TspW[i,j,s]<-TrW[i,j,s]*surv[i,j,5,1]*kE[i,j,5]		
...
# }   
    ...
    # River fishery
        ...
        cr_predW[i,j] <- TrWtot[i,j]* HrW[i,j]* reportrW         
        psi_rW[i,j]<-rrW[j]/(rrW[j]+cr_predW[i,j])
        cr_ObsW[i,j] ~ dnegbin(psi_rW[i,j], rrW[j])
     
        cr_predRsp[i,j] <- TrRsptot[i,j]*HrW[i,j]* reportrW               # note uses reared surv but wild HR        
    ...
    # Coastal fishery
        ...
        NrW_all[i,j]<- sum(NladderW[i,j,avail_r])+sum(NladderRsp[i,j,1:2])	  #1:13, 16  #08.06.2017 Ume included
